...
!  12/21 Restarting info added for TI
!
!  Conditions of use:
!
...
!
  ...
  if (index(word,'lambda').eq.1) goto 480
  if (index(word,'dudl').eq.1) goto 490
  return
!*************
!  Timestep  *
!*************
...
470 if (nfloat.eq.0) then
  ...
  return
!***************************
!  Lambda settings for TI  *
!***************************
480 if (nfloat.lt.4) then
    call outerror('input values are missing for lambda option',iline)
    call stopnow('mdword')
  endif
  units = 1.0_dp
  if (nword.gt.1) then
  ...
  lambda_initial = abs(floats(1))
  lambda_final = abs(floats(2))
  nlambda = abs(nint(floats(3)))
  tlambda = abs(floats(4))*units
!
!  Sanity checks on values
!
  if (lambda_initial.gt.1.0_dp) then
    call outerror('initial lambda cannot be greater than 1',iline)
    call stopnow('mdword')
  endif
  if (lambda_final.gt.1.0_dp) then
    call outerror('final lambda cannot be greater than 1',iline)
    call stopnow('mdword')
  endif
  if (nlambda.eq.0.and.abs(lambda_initial-lambda_final).gt.1.0d-4) then
    call outerror('no. of steps for lambda cannot be 0 when initial and final value differ',iline)
    call stopnow('mdword')
  endif
!
!  Check memory
!
  if (nlambda+1.gt.maxlambda) then
    maxlambda = nlambda + 1
...
!
  ...
  return
!*************************************
!  dUdlambda history for restarting  *
!*************************************
490 if (nfloat.lt.1) then
    call outerror('number of dUdlambda values to be read is missing',iline)
    call stopnow('mdword')
  elseif (nfloat.lt.2) then
    call outerror('current step number within lambda is missing',iline)
    call stopnow('mdword')
  endif
  nlambdanow = nint(abs(floats(1)))
  ntinow = nint(abs(floats(2)))
!
!  Check that memory has been initialised to hold this many values
!
  if (nlambdanow+1.gt.maxlambda) then
    maxlambda = nlambdanow + 1
    call changemaxlambda
  endif
!
!  Read dUdlambda values
!
  do i = 1,nlambdanow
    line = '  '
    read(nru,'(a)') line
    ...
    call linepro(nru,line,iline)
    if (nfloat.lt.2) then
      call outerror('insufficient values for dUdlambda in input',iline)
      call stopnow('mdword')
    endif
    dUdlambda(i) = floats(2)
  enddo
!
  lwordok = .true.
  return
