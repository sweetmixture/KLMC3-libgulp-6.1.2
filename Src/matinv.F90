...
!*************************************************************************
  subroutine matrix_inversion_library(n,nmin,ldm,nblock,matrix,nproc0,lsymmetric,ifail)
!
...
!   7/17 nproc0 added
!   2/18 Trace added
!   7/21 Symmetric or asymmetric option added
!
!  Conditions of use:
!
...
!
  ...
  logical,     intent(in)                      :: lsymmetric    ! If true then matrix is symmetric
...
!  Factorise matrix
!
      call dgetrf(n,n,matrix,ldm,ipivot,ifail)
      if (ifail.eq.0) then
!
!  Form inverse
!
        call dgetri(n,matrix,ldm,ipivot,wrk,lwrk,ifail)
      endif
!
!  Free workspace
!
      ...
      do i = 1,ntransfer
        if (procid.eq.nnoderecv(i)) then
          ...
          call MPI_IRecv(matrix2(1,3*(nnoderecvptr(i)-1)+1),3*ldm,MPI_double_precision,nnodesend(i), &
                         i,MPI_Comm_GULP,Request(nlocalcomm),MPIerror)
...
!
      call matrix_inversion_library(n,1_i4,ldm,3_i4*nblocksize,matrix2,0_i4,.true.,ifail)
!
...
!  Parallel inversion using scalapack  *
!***************************************
      call matrix_inversion_library(n,nmin,ldm,3_i4*nblocksize,matrix,nproc0,.true.,ifail)
    endif
#else
    call outerror('Parallel matrix inversion called without MPI',0_i4)
...
!****************************************
    call matrix_inversion_library(n,nmin,ldm,3_i4*nblocksize,matrix,0_i4,.true.,ifail)
  endif
!
  t2p = g_cpu_time()
#ifdef TRACE
