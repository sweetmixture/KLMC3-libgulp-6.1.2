...
!  OpenMM taper  *
!*****************
  subroutine openmmtaper(r,rmin,rmax,f,dfdr,d2fdr2,d3fdr3,lgrad1,lgrad2,lgrad3)
!
!  Subroutine to calculate the taper function and derivatives 
!  using the OpenMM interpolation form 
!
!  On entry : 
!
!  r               = current distance for which the taper function
...
!********************
    ...
    x2 = x*x
    x3 = x*x*x
    f = 1.0_dp - x3*(6.0_dp*x2 - 15.0_dp*x + 10.0_dp)
    if (lgrad1) then
      dfdr = - 30.0_dp*x2*(x2 - 2.0_dp*x + 1.0_dp)*rrdiff
      if (lgrad2) then
        d2fdr2 = - x*(120.0_dp*x2 - 180.0_dp*x + 60.0_dp)*rrdiff*rrdiff
        if (lgrad3) then
          d3fdr3 = - (360.0_dp*x2 - 360.0_dp + 60.0_dp)*rrdiff*rrdiff**rrdiff
        endif
      endif
    endif
  endif
#ifdef TRACE
  call trace_out('openmmtaper')
#endif
!
  return
  end
