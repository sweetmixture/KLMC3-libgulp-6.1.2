...
!
  ...
  use four,          only : ltib4body, ltif4body
  use iochannels
  use m_ti
  use parallel
#ifdef TRACE
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    cosnphi = cos(rn*phi)
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor*(1.0_dp + isgn*cosnphi*cphi0)
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor*(1.0_dp + isgn*cosnphi*cphi0)
        rkforl = rkfor*(1.0_dp - lambda)
      else
        rkforl = rkfor
      endif
    else
    ...
    efor = rkforl*(1.0_dp + isgn*cosnphi*cphi0)
    if (lphi0non0) then
      sinnphi = sin(rn*phi)
      sinphi = sin(phi)
!
      if (lti) then
        if (ltif4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor*isgn*sinnphi*sphi0
        elseif (ltib4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor*isgn*sinnphi*sphi0
        endif
      endif
!
      efor = efor + rkforl*isgn*sinnphi*sphi0
!
...
!
    ...
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor
        rkforl = rkfor*(1.0_dp - lambda)
    ...
    do i = 1,norder
      ...
      if (lti) then
        if (ltif4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + fp*cphi1
          fp = fp*lambda
        elseif (ltib4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - fp*cphi1
          fp = fp*(1.0_dp - lambda)
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor*r122*ctrm1*ctrm2
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor*r122*ctrm1*ctrm2
        rkforl = rkfor*(1.0_dp - lambda)
...
!
    efor = rkforl*r122*ctrm1*ctrm2
    if (lk4) then
      rkfor4 = fpoly(1)
      if (lti) then
        if (ltif4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor4*r122*r122*ctrm12*ctrm22
          rkfor4 = rkfor4*lambda
        elseif (ltib4body(nptr4)) then
          dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor4*r122*r122*ctrm12*ctrm22
          rkfor4 = rkfor4*(1.0_dp - lambda)
        endif
      endif
...
!
    ...
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + sin1*sin1*sin3*sin3*(e0a + e0b*sin1nm2*sin3nm2*cosnphi)
        e0a = e0a*lambda
        e0b = e0b*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - sin1*sin1*sin3*sin3*(e0a + e0b*sin1nm2*sin3nm2*cosnphi)
        e0a = e0a*(1.0_dp - lambda)
        e0b = e0b*(1.0_dp - lambda)
      endif
    endif
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + 0.5_dp*rkfor*(phi-phi0)**2
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - 0.5_dp*rkfor*(phi-phi0)**2
        rkforl = rkfor*(1.0_dp - lambda)
...
!
        ...
        do i = 1,6
          do j = i,6
            ii = ii + 1
            e2d(ii) = rkforl*phi1d(i)*phi1d(j) + e1*phi2d(ii)
          enddo
        enddo
        if (lgrad3loc) then
!
          ...
          do i=1,6
            do j=i,6
              ...
              do k=j,6  
                ...
                ik=kb(k,i)
                jk=kb(k,j)
                e3d(ii) = e1*phi3d(ii) + rkforl*(phi1d(i)*phi2d(jk) + phi1d(j)*phi2d(ik) + phi1d(k)*phi2d(ij))
              enddo
            enddo
          enddo
        endif
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + et0*ee0
        rkforl = rkfor*lambda
        et0 = et0*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - et0*ee0
        rkforl = rkfor*(1.0_dp - lambda)
        et0 = et0*(1.0_dp - lambda)
      else
        rkforl = rkfor
      endif
    else
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkfor*delth1*delth2*cosphi
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkfor*delth1*delth2*cosphi
        rkforl = rkfor*(1.0_dp - lambda)
...
!
    efor = rkforl*delth1*delth2*cosphi
    if (lgrad1loc) then
      do i = 1,6
        e1d(i) = rkforl*((theta11d(i)*delth2 + delth1*theta21d(i))*cosphi + delth1*delth2*cosp1d(i))
      enddo
      if (lgrad2loc) then
        ii = 0
        do i = 1,6
          do j = i,6
            ii = ii + 1
            e2d(ii) = rkforl*((theta12d(ii)*delth2 + delth1*theta22d(ii))*cosphi + delth1*delth2*cosp2d(ii))
            e2d(ii) = e2d(ii) + rkforl*(theta11d(i)*theta21d(j) + theta11d(j)*theta21d(i))*cosphi
            e2d(ii) = e2d(ii) + rkforl*(theta11d(i)*delth2 + delth1*theta21d(i))*cosp1d(j)
            e2d(ii) = e2d(ii) + rkforl*(theta11d(j)*delth2 + delth1*theta21d(j))*cosp1d(i)
          enddo
        enddo
        if (lgrad3loc) then
          ii = 0
          do i = 1,6
            do j = i,6
              ...
              do k = j,6
                ...
                ik = kb(k,i)
                jk = kb(k,j)
                e3d(ii) = rkforl*((theta13d(ii)*delth2 + delth1*theta23d(ii))*cosphi + delth1*delth2*cosp3d(ii))
                e3d(ii) = e3d(ii) + rkforl*(theta11d(i)*theta21d(j) + theta11d(j)*theta21d(i))*cosp1d(k)
                e3d(ii) = e3d(ii) + rkforl*(theta11d(i)*theta21d(k) + theta11d(k)*theta21d(i))*cosp1d(j)
                e3d(ii) = e3d(ii) + rkforl*(theta11d(j)*theta21d(k) + theta11d(k)*theta21d(j))*cosp1d(i)
                e3d(ii) = e3d(ii) + rkforl*theta12d(ij)*(delth2*cosp1d(k) + theta21d(k)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*theta12d(ik)*(delth2*cosp1d(j) + theta21d(j)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*theta12d(jk)*(delth2*cosp1d(i) + theta21d(i)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*theta22d(ij)*(delth1*cosp1d(k) + theta11d(k)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*theta22d(ik)*(delth1*cosp1d(j) + theta11d(j)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*theta22d(jk)*(delth1*cosp1d(i) + theta11d(i)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(ij)*(delth1*theta21d(k) + theta11d(k)*delth2)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(ik)*(delth1*theta21d(j) + theta11d(j)*delth2)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(jk)*(delth1*theta21d(i) + theta11d(i)*delth2)
              enddo
            enddo
          enddo
        endif
...
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rkover3*(3.0_dp - cospsi1 - cospsi2 - cospsi3)
        rkover3 = rkover3*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rkover3*(3.0_dp - cospsi1 - cospsi2 - cospsi3)
        rkover3 = rkover3*(1.0_dp - lambda)
      endif
    endif
    ...
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + &
          rkover3*((cospsi1 - cosk0)**2 + (cospsi2 - cosk0)**2 + (cospsi3 - cosk0)**2)
        rkover3 = rkover3*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - &
          rkover3*((cospsi1 - cosk0)**2 + (cospsi2 - cosk0)**2 + (cospsi3 - cosk0)**2)
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + 0.5_dp*rkfor*(1.0_dp - cosnphi*cphi0)
        rkforl = rkfor*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - 0.5_dp*rkfor*(1.0_dp - cosnphi*cphi0)
        rkforl = rkfor*(1.0_dp - lambda)
...
!
    efor = 0.5_dp*rkforl*(1.0_dp - cosnphi*cphi0)
    if (lgrad1loc) then
!
!  First derivatives of energy
!
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + k1*ttrm1*ttrm2 + k2*ttrm1*ttrm3 + k3*ttrm2*ttrm3
        k1 = k1*lambda
        k2 = k2*lambda
        k3 = k3*lambda
      elseif (ltib4body(nptr4)) then
...
!
    if (lti) then
      if (ltif4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + &
          rkover3*(3.0_dp*c0 + c1*(cospsi1 + cospsi2 + cospsi3) + c2*(cos2psi1 + cos2psi2 + cos2psi3))
        rkover3 = rkover3*lambda
      elseif (ltib4body(nptr4)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - &
          rkover3*(3.0_dp*c0 + c1*(cospsi1 + cospsi2 + cospsi3) + c2*(cos2psi1 + cos2psi2 + cos2psi3))
...
! 
    ...
    if (lgrad1loc) then
      do i = 1,6
        e1d(i) = rkforl*((cos11d(i)*delth2 + delth1*cos21d(i))*cosphi + delth1*delth2*cosp1d(i))
      enddo
      if (lgrad2loc) then
        ii = 0
        do i = 1,6
          do j = i,6
            ii = ii + 1
            e2d(ii) = rkforl*((cos12d(ii)*delth2 + delth1*cos22d(ii))*cosphi + delth1*delth2*cosp2d(ii))
            e2d(ii) = e2d(ii) + rkforl*(cos11d(i)*cos21d(j) + cos11d(j)*cos21d(i))*cosphi
            e2d(ii) = e2d(ii) + rkforl*(cos11d(i)*delth2 + delth1*cos21d(i))*cosp1d(j)
            e2d(ii) = e2d(ii) + rkforl*(cos11d(j)*delth2 + delth1*cos21d(j))*cosp1d(i)
          enddo
        enddo
        if (lgrad3loc) then
          ii = 0
          do i = 1,6
            do j = i,6
              ...
              do k = j,6
                ...
                ik = kb(k,i)
                jk = kb(k,j)
                e3d(ii) = rkforl*((cos13d(ii)*delth2 + delth1*cos23d(ii))*cosphi + delth1*delth2*cosp3d(ii))
                e3d(ii) = e3d(ii) + rkforl*(cos11d(i)*cos21d(j) + cos11d(j)*cos21d(i))*cosp1d(k)
                e3d(ii) = e3d(ii) + rkforl*(cos11d(i)*cos21d(k) + cos11d(k)*cos21d(i))*cosp1d(j)
                e3d(ii) = e3d(ii) + rkforl*(cos11d(j)*cos21d(k) + cos11d(k)*cos21d(j))*cosp1d(i)
                e3d(ii) = e3d(ii) + rkforl*cos12d(ij)*(delth2*cosp1d(k) + cos21d(k)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cos12d(ik)*(delth2*cosp1d(j) + cos21d(j)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cos12d(jk)*(delth2*cosp1d(i) + cos21d(i)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cos22d(ij)*(delth1*cosp1d(k) + cos11d(k)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cos22d(ik)*(delth1*cosp1d(j) + cos11d(j)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cos22d(jk)*(delth1*cosp1d(i) + cos11d(i)*cosphi)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(ij)*(delth1*cos21d(k) + cos11d(k)*delth2)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(ik)*(delth1*cos21d(j) + cos11d(j)*delth2)
                e3d(ii) = e3d(ii) + rkforl*cosp2d(jk)*(delth1*cos21d(i) + cos11d(i)*delth2)
              enddo
            enddo
          enddo
        endif
