...
!   2/22 Equipartition free energy added
!
    ...
    if (lnozeropt) then
      if (lequipartition) then
        do iloc = mcvminloc,mcvmaxloc
          i = mlocptr(iloc)
          fetrm = fetrm + log(freq(i,k-nlkpt+1)*cmfact)
          dfdw2(iloc) = 0.5_dp*wk*rkptfct/freq(i,k-nlkpt+1)**2
!
!  Sqrt dfdw2 so that it can be convoluted into the eigenvectors to save time on the projection of the third derivatives
!
...
!
      ...
      if (lequipartition) then
        do iloc = mcvminloc,mcvmaxloc
          ...
          fetrm = fetrm + log(freq(i,k-nlkpt+1)*cmfact)
          dfdw2(iloc) = 0.25_dp*wk*rkptcmfct/freq(i,k-nlkpt+1) + 0.5_dp*wk*rkptfct/freq(i,k-nlkpt+1)**2
!
!  Sqrt dfdw2 so that it can be convoluted into the eigenvectors to save time on the projection of the third derivatives
!
          dfdw2(iloc) = sqrt(dfdw2(iloc))*fscale
