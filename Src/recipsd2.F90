...
!
  ...
  if (status/=0) call outofmemory('recipsd2','xdrl')
  allocate(ydrl(numat),stat=status)
  if (status/=0) call outofmemory('recipsd2','ydrl')
  allocate(zdrl(numat),stat=status)
  if (status/=0) call outofmemory('recipsd2','zdrl')
!
!  Setup
!
  eta4 = 0.25_dp/eta
                ...
                if (lgrad2) then
                  ...
                  do kk = 1,6
                    ...
                    if (.not.lfreeze.or.lopi) then
                      ...
                      if (lrigid) then
                        derv3(ix,kk) = derv3(ix,kk) + xrkk*d3kk + cosq*xrkk*dGrds1 + d1trm*( &
                          xrkk*d2rxyzdsdx(kk,1,1) + dgds(iv,1,kk) + yrkk*d2rxyzdsdx(kk,2,1) + zrkk*d2rxyzdsdx(kk,3,1))
                        derv3(iy,kk) = derv3(iy,kk) + yrkk*d3kk + cosq*yrkk*dGrds1 + d1trm*( &
                          xrkk*d2rxyzdsdx(kk,1,2) + yrkk*d2rxyzdsdx(kk,2,2) + dgds(iv,2,kk) + zrkk*d2rxyzdsdx(kk,3,2))
                        derv3(iz,kk) = derv3(iz,kk) + zrkk*d3kk + cosq*zrkk*dGrds1 + d1trm*( &
              ...
              if (lsg1) then
                ...
                if (lrigid) then
                  do k = 1,6
                    dGrds = xrkk*drxyzds(k,1) + yrkk*drxyzds(k,2) + zrkk*drxyzds(k,3) + &
                            xd*dgds(iv,1,k) + yd*dgds(iv,2,k) + zd*dgds(iv,3,k)
                    strderv(k) = strderv(k) - 0.5_dp*sinq*dGrds
                  enddo
                endif
                do k = 1,6
                  strderv(k) = strderv(k) + strm1*dg2ds(iv,k)
...
!
  ...
  if (status/=0) call deallocate_error('recipsd2','zdrl')
  deallocate(ydrl,stat=status)
  if (status/=0) call deallocate_error('recipsd2','ydrl')
  deallocate(xdrl,stat=status)
  if (status/=0) call deallocate_error('recipsd2','xdrl')
