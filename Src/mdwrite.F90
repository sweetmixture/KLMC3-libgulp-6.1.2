...
!
  if (lxyz.and.lxyzmovie) then
    ...
    if (lxyz_extended) then
      write(steps,'(i12)') ncstep
      if (ndim.eq.3) then
        write(vectors,'(9f10.5)') rv(1,1),rv(2,1),rv(3,1),rv(1,2),rv(2,2),rv(3,2),rv(1,3),rv(2,3),rv(3,3)
        xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T T T" Time='&
          //trim(adjustl(steps))
      elseif (ndim.eq.2) then
        write(vectors,'(6f10.5)') rv(1,1),rv(2,1),rv(3,1),rv(1,2),rv(2,2),rv(3,2)
        xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T T F" Time='&
          //trim(adjustl(steps))
      elseif (ndim.eq.1) then
        write(vectors,'(3f10.5)') rv(1,1),rv(2,1),rv(3,1)
        xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T F F" Time='&
          //trim(adjustl(steps))
      else
        xyzline = 'Properties=species:S:1:pos:R:3:charges:R:1 pbc="F F F" Time='//trim(adjustl(steps))
      endif
      write(33,'(a)') trim(adjustl(xyzline))
      do i = 1,numat
        inat = nat(i)
        if (inat.le.maxele) then
          call label(inat,0_i4,lab)
          write(33,'(a4,1x,4f20.9)') lab,xalat(i),yalat(i),zalat(i),qf(i)
        endif
      enddo
    else
