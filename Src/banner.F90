...
!
!  Julian Gale, Curtin University, October 2022
!
  use iochannels
  use parallel
#ifdef TRACE
...
!******************
  if (ioproc) then
#ifdef ACCELRYS
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                       GENERAL UTILITY LATTICE PROGRAM  v6.1                  *'')')
    ...
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                     BIOVIA Materials Studio 2022 Release                     *'')')
    write(ioout,'(''********************************************************************************'')')
#else
    write(ioout,'(''********************************************************************************'')')
    ...
    write(ioout,'(''*                    Curtin University, Western Australia                      *'')')
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''* Version = 6.1.2 * Last modified =   3rd November 2022                        *'')')
    write(ioout,'(''********************************************************************************'')')
#endif
  endif
#ifdef TRACE
