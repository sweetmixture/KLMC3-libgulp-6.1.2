...
!
  ...
  use current
  use element
  use frequencies,    only : rfmass, fphase
...
!
  ...
  real(dp)                                     :: cmfact
  real(dp)                                     :: drQ(3,3)
  real(dp),    dimension(:),   allocatable     :: egi
...
!
  allocate(egi(mtv),stat=status)
  if (status/=0) call outofmemory('peigen','egi')
  allocate(egr(mtv),stat=status)
  if (status/=0) call outofmemory('peigen','egr')
  allocate(enorm(mtv),stat=status)
  if (status/=0) call outofmemory('peigen','enorm')
!
...
!************************************************************************
  if (.not.leigenvectors_original) then
    do m = 1,mtv
      enorm(m) = 0.0_dp
      do i = 1,mtv
         enorm(m) = enorm(m) + (eigr(i,m)**2 + eigi(i,m)**2)*rfmass(i)**2
      enddo
  ...
  if (leig) then
    do m = 1,mtv
      ...
      if (leigenvectors_original) then
        egr(1:mtv) = eigr(1:mtv,m)
        egi(1:mtv) = eigi(1:mtv,m)
      else
        do n = 1,mtv
          egr(n) = eigr(n,m)*enorm(m)*rfmass(n)
          egi(n) = eigi(n,m)*enorm(m)*rfmass(n)
        enddo
      endif
      if (.not.leigenvectors_phase) then
        do n = 1,mtv
          egc = dcmplx(egr(n),egi(n))*fphase(n)
          egr(n) = dble(egc)
          egi(n) = aimag(egc)
        enddo
      endif
      ...
      do i = 1,nphonatc,nghostcell
        write(53,'(3f10.6,4x,3f10.6)') (egr(ii+j),j=1,3),(egi(ii+k),k=1,3)
        ii = ii + 3
      enddo
      if (lrigid) then
        do i = 1,nphonatm,nghostcell
          write(53,'(3f10.6,4x,3f10.6)') (egr(ii+j),j=1,6),(egi(ii+k),k=1,3)
          ii = ii + 3
          write(53,'(3f10.6,4x,3f10.6)') (egr(ii+j),j=1,6),(egi(ii+k),k=1,3)
          ii = ii + 3
        enddo
...
!
      do i = 1,nphonatc,nghostcell
        ...
        write(54,'(2i5,2f16.12,4x,2f16.12,4x,2f16.12)') m,ig, &
          egr(ii+1),egi(ii+1),egr(ii+2),egi(ii+2),egr(ii+3),egi(ii+3)
        ii = ii + 3
      enddo
      if (lrigid) then
        do i = 1,nphonatm,nghostcell
    ...
    if (igroup.gt.0) then
      do i = 1,igroup
        ...
        do j = 1,njloop
          do k = 1,3
            if (leigenvectors_original) then
              egrl(1:3,k) = eigr(indj+1:indj+3,indi+k)
              egil(1:3,k) = eigi(indj+1:indj+3,indi+k)
            else
              do n = 1,3
                egrl(n,k) = eigr(indj+n,indi+k)*enorm(indi+k)*rfmass(indj+n)
                egil(n,k) = eigi(indj+n,indi+k)*enorm(indi+k)*rfmass(indj+n)
              enddo
            endif
!
            if (.not.leigenvectors_phase) then
              do n = 1,3
                egc = dcmplx(egrl(n,k),egil(n,k))*fphase(indj+n)
                egrl(n,k) = dble(egc)
                egil(n,k) = aimag(egc)
              enddo
            endif
          enddo
!
          write(ioout,'(i6,'' x '',4x,6f10.6)') j,(egrl(1,k),egil(1,k),k=1,3)
          write(ioout,'(i6,'' y '',4x,6f10.6)') j,(egrl(2,k),egil(2,k),k=1,3)
          write(ioout,'(i6,'' z '',4x,6f10.6)') j,(egrl(3,k),egil(3,k),k=1,3)
          indj = indj + 3
        enddo
        if (lrigid) then
          do j = 1,nphonatm
            do k = 1,3
              if (leigenvectors_original) then
                egrl(1:6,k) = eigr(indj+1:indj+6,indi+k)
                egil(1:6,k) = eigi(indj+1:indj+6,indi+k)
              else
                do n = 1,6
                  egrl(n,k) = eigr(indj+n,indi+k)*enorm(indi+k)*rfmass(indj+n)
                  egil(n,k) = eigi(indj+n,indi+k)*enorm(indi+k)*rfmass(indj+n)
...
!
            ...
            enddo
!
            write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(egrl(1,k),egil(1,k),k=1,3)
            write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(egrl(2,k),egil(2,k),k=1,3)
            write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(egrl(3,k),egil(3,k),k=1,3)
            write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(egrl(4,k),egil(4,k),k=1,3)
            write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(egrl(5,k),egil(5,k),k=1,3)
            write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(egrl(6,k),egil(6,k),k=1,3)
            indj = indj + 6
          enddo
        endif
        indi = indi + 3
...
!
        ...
        enddo
!
        write(ioout,'(i6,'' x '',4x,6f10.6)') j,(egrl(1,k),egil(1,k),k=1,iresid)
        write(ioout,'(i6,'' y '',4x,6f10.6)') j,(egrl(2,k),egil(2,k),k=1,iresid)
        write(ioout,'(i6,'' z '',4x,6f10.6)') j,(egrl(3,k),egil(3,k),k=1,iresid)
        indj = indj + 3
      enddo
      if (lrigid) then
        do j = 1,nphonatm
...
!
          ...
          enddo
!
          write(ioout,'(i6,'' Tx'',4x,6f10.6)') j,(egrl(1,k),egil(1,k),k=1,iresid)
          write(ioout,'(i6,'' Ty'',4x,6f10.6)') j,(egrl(2,k),egil(2,k),k=1,iresid)
          write(ioout,'(i6,'' Tz'',4x,6f10.6)') j,(egrl(3,k),egil(3,k),k=1,iresid)
          write(ioout,'(i6,'' R1'',4x,6f10.6)') j,(egrl(4,k),egil(4,k),k=1,iresid)
          write(ioout,'(i6,'' R2'',4x,6f10.6)') j,(egrl(5,k),egil(5,k),k=1,iresid)
          write(ioout,'(i6,'' R3'',4x,6f10.6)') j,(egrl(6,k),egil(6,k),k=1,iresid)
          indj = indj + 6
        enddo
      endif
    endif
...
!
  ...
  if (status/=0) call deallocate_error('peigen','enorm')
  deallocate(egr,stat=status)
  if (status/=0) call deallocate_error('peigen','egr')
  deallocate(egi,stat=status)
  if (status/=0) call deallocate_error('peigen','egi')
  if (linten.or.lmsd.or.lout) then
