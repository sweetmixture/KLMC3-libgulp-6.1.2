...
!  iodfset       => channel number for displacement and force file
!
...
!  10/21 Modified for alamode version 1.3.0
!
!  Conditions of use:
!
...
!
  ...
  character(len=maxwordlength)                     :: dfsetfile_c
  character(len=maxwordlength)                     :: dfsetfile_h
...
!
  dfsetfile_h = trim(prefix)//'_dfset_harm.dat'
  dfsetfile_c = trim(prefix)//'_dfset_cubic.dat'
!###############################################################################
  ...
  if (ioproc) then
    ...
    if (lmode_all.or.lmode_displace) then
      if (lalamode_fc2) then
        ...
        write(ioout,'(''    Displacements and forces to be written to '',a)') trim(dfsetfile_h)
      endif
      if (lalamode_fc3) then
        ...
        write(ioout,'(''    Displacements and forces to be written to '',a)') trim(dfsetfile_c)
      endif
    endif
    write(ioout,'(/)')
  endif
...
!
        ...
        if (lmode_all.or.lmode_displace) then
!
!  Open output file for displacements and forces
!
          open(iodfset,file=dfsetfile_h,status='unknown',form='formatted')
        endif
!
...
!
          ...
          if (ioproc) then
!
!  Output displacements and forces to file
!
            do i = 1,ncore
              ii = ncoptr(i)
              write(iodfset,'(3f21.14,3e19.11)') (dispxyz(j,i),j=1,3),xdrv(ii)*fconv,ydrv(ii)*fconv,zdrv(ii)*fconv
            enddo
          endif
!
...
!  Open output file for displacements and forces
!
          open(iodfset,file=dfsetfile_c,status='unknown',form='formatted')
        endif
!
!  Read in header from file with Basis
!
...
!  Fitting block
!
        write(ioalamode,'(''&optimize'')')
        write(ioalamode,'(''  NDATA = '',i6)') ndisp2
        write(ioalamode,'(''  DFSET = '',a)') dfsetfile_h
        write(ioalamode,'(''/'',/)')
!
!  Interaction block
!
...
!
        ...
        write(ioalamode,'(''  DFSET = '',a)') dfsetfile_c
        write(ioalamode,'(''/'',/)')
!
!  Interaction block
!
...
!   7/20 Version number now obtained from alm_fit.log as name has changed
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
