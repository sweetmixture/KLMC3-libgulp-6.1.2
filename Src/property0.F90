...
!   1/18 Trace added
!   3/21 Warning added for Einstein crystal
!   3/21 Output corrected to specify log of partition functions
!   7/21 Call to matrix_inversion_library changed
!  11/21 Correction to rotational partition of sqrt(pi) that is
!        missing from formula in textbook that was being used
!   1/22 Parallel handling of shells added
!   2/22 Gibbs and Helmholtz free energies now output for translation
!
!  Conditions of use:
!
...
!
  use g_constants,      only : avogadro, evtoj, boltz, kjmtoev, pi
  use control,          only : lraman
  use current
  use derivatives,      only : derv2, dervi, maxd2, maxd2u
  use element
...
!
  ...
  real(dp)                                       :: r3(3)
  real(dp)                                       :: r33(3,3)
  real(dp)                                       :: rkT             ! Boltzmann's constant x temperature in eV
...
!
    ...
    if (nprocs.gt.1) then
      ...
      if (status/=0) call outofmemory('property0','qshellnode')
    endif
!
...
!
    if (nprocs.gt.1) then
      ...
      if (status/=0) call deallocate_error('property0','qshellnode')
...
!
        ...
        lnZr = 0.5_dp*log(inertia_product) + 1.5_dp*log(temperature) - log(dble(symnocfg(ncf))) + 1.5_dp*1.418_dp + 0.5_dp*log(pi)
      endif
      Gr = - rkT*lnZr
    endif
  endif
...
!**********************
  if (lprint.and.ioproc) then
    ...
    if (temperature.gt.1.0d-3) then
      ...
      if (leinstein) then
        write(ioout,'(''  NB: Einstein crystal is being used and so these properties are not valid!'')')
      endif
      write(ioout,'(''-------------------------------------------------------------------------------'')')
      if (numat.gt.1) then
        write(ioout,'(''  Log Rotational    partition function  = '',f18.6)') lnZr
        write(ioout,'(''  Log Translational partition function  = '',f18.6)') lnZt
        write(ioout,'(''  Rotational free energy                = '',f18.6,'' eV'')') Gr
        write(ioout,'(''                                        = '',f18.6,'' kJ/mol'')') Gr/kjmtoev
        write(ioout,'(''  Translational free energy (Helmholtz) = '',f18.6,'' eV'')') Gt + Gn
        write(ioout,'(''                            (Helmholtz) = '',f18.6,'' kJ/mol'')') (Gt + Gn)/kjmtoev
        write(ioout,'(''  Translational free energy (Gibbs)     = '',f18.6,'' eV'')') Gt + Gn + rkT
        write(ioout,'(''                            (Gibbs)     = '',f18.6,'' kJ/mol'')') (Gt + Gn + rkT)/kjmtoev
        write(ioout,'(''  Total free energy         (Helmholtz) = '',f18.6,'' eV'')') Gt + Gr + Gn
        write(ioout,'(''                            (Helmholtz) = '',f18.6,'' kJ/mol'')') (Gt + Gr + Gn)/kjmtoev
        write(ioout,'(''  Total free energy         (Gibbs)     = '',f18.6,'' eV'')') Gt + Gr + Gn + rkT
        write(ioout,'(''                            (Gibbs)     = '',f18.6,'' kJ/mol'')') (Gt + Gr + Gn + rKT)/kjmtoev
      else
        write(ioout,'(''  Log Translational partition function  = '',f18.6)') lnZt
        write(ioout,'(''  Total free energy         (Helmholtz) = '',f18.6,'' eV'')') Gt + Gn
        write(ioout,'(''                            (Helmholtz) = '',f18.6,'' kJ/mol'')') (Gt + Gn)/kjmtoev
        write(ioout,'(''  Total free energy         (Gibbs)     = '',f18.6,'' eV'')') Gt + Gn + rkT
        write(ioout,'(''                            (Gibbs)     = '',f18.6,'' kJ/mol'')') (Gt + Gn + rKT)/kjmtoev
      endif
      write(ioout,'(''-------------------------------------------------------------------------------'',/)')
    endif
