  subroutine psumall(eatom,ereal,erecip,ec6,eqeq,eattach,esregion12,esregion2, &
    ...
    eplumed,eboQself,eval,egfnff,lgrad1,lgrad2,lsym)
!
...
!
  if (latomicstress) then
    allocate(sum(max(33+nregs+n,4*n+3*maxregion+nstrains*(numat+1)+36)),stat=status)
    if (status/=0) call outofmemory('psumall','sum')
    allocate(sum0(max(33+nregs+n,4*n+3*maxregion+nstrains*(numat+1)+36)),stat=status)
    if (status/=0) call outofmemory('psumall','sum0')
  else
    allocate(sum(max(33+nregs+n,4*n+3*maxregion+nstrains+36)),stat=status)
    if (status/=0) call outofmemory('psumall','sum')
    allocate(sum0(max(33+nregs+n,4*n+3*maxregion+nstrains+36)),stat=status)
    if (status/=0) call outofmemory('psumall','sum0')
  endif
!****************
!  Global sums  *
