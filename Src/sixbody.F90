...
!
  ...
  use m_ti
  use parallel
  use six,           only : ltib6body, ltif6body
#ifdef TRACE
...
!
    if (lti) then
      if (ltif6body(nptr6)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) + rksix*oopda*oopdb
        rksixl = rksix*lambda
      elseif (ltib6body(nptr6)) then
        dUdlambda(nlambdanow) = dUdlambda(nlambdanow) - rksix*oopda*oopdb
        rksixl = rksix*(1.0_dp - lambda)
      else
        rksixl = rksix
      endif
    else
...
!
      ...
      do k = 1,15
        e1d(k) = rksixl*(oopd1a(k)*oopdb + oopda*oopd1b(k))
      enddo
      if (lgrad2) then
!
!  Second derivatives
!
        ...
        do i = 1,15
          do j = i,15
            ii = ii + 1
            e2d(ii) = rksixl*(oopd2a(ii)*oopdb + oopda*oopd2b(ii))
            e2d(ii) = e2d(ii) + rksixl*(oopd1a(i)*oopd1b(j) + oopd1a(j)*oopd1b(i))
          enddo
        enddo
        if (lgrad3) then
!
          ...
          do i = 1,15
            do j = i,15
              ...
              do k = j,15
                ...
                ik = kb2(k,i)
                jk = kb2(k,j)
                e3d(ii) = rksixl*(oopd3a(ii)*oopdb + oopda*oopd3b(ii))
                e3d(ii) = e3d(ii) + rksixl*(oopd2a(jk)*oopd1b(i) + oopd1a(i)*oopd2b(jk))
                e3d(ii) = e3d(ii) + rksixl*(oopd2a(ik)*oopd1b(j) + oopd1a(j)*oopd2b(ik))
                e3d(ii) = e3d(ii) + rksixl*(oopd2a(ij)*oopd1b(k) + oopd1a(k)*oopd2b(ij))
              enddo
            enddo
          enddo
        endif
