...
!
  ...
  if (status/=0) call outofmemory('gfnff_phond','cn')
  allocate(dcn(numat),stat=status)
  if (status/=0) call outofmemory('gfnff_phond','dcn')
  allocate(d2cn(numat),stat=status)
  if (status/=0) call outofmemory('gfnff_phond','d2cn')
  allocate(gw(maxc6ref,numat),stat=status)
  if (status/=0) call outofmemory('gfnff_phond','gw')
!
  allocate(dgwdcn(maxc6ref,numat),stat=status)
  if (status/=0) call outofmemory('gfnff_phond','dgwdcn')
!
  allocate(d2gwdcn2(maxc6ref,numat),stat=status)
  if (status/=0) call outofmemory('gfnff_phond','d2gwdcn2')
!
  if (ldohbcn) then
    allocate(cn_hb(numat),stat=status)
    if (status/=0) call outofmemory('gfnff_phond','cn_hb')
  endif
!
...
!
        if (lilocal) then
          derv2(jxf,ix) = derv2(jxf,ix) - d2eijdr2*xji*xji
          derv2(jyf,ix) = derv2(jyf,ix) - d2eijdr2*xji*yji
          derv2(jzf,ix) = derv2(jzf,ix) - d2eijdr2*xji*zji
          derv2(jxf,iy) = derv2(jxf,iy) - d2eijdr2*yji*xji
          derv2(jyf,iy) = derv2(jyf,iy) - d2eijdr2*yji*yji
          derv2(jzf,iy) = derv2(jzf,iy) - d2eijdr2*yji*zji
          derv2(jxf,iz) = derv2(jxf,iz) - d2eijdr2*zji*xji
          derv2(jyf,iz) = derv2(jyf,iz) - d2eijdr2*zji*yji
          derv2(jzf,iz) = derv2(jzf,iz) - d2eijdr2*zji*zji
...
!
        if (ljlocal) then
          derv2(ixf,jx) = derv2(ixf,jx) - d2eijdr2*xji*xji
          derv2(iyf,jx) = derv2(iyf,jx) - d2eijdr2*xji*yji
          derv2(izf,jx) = derv2(izf,jx) - d2eijdr2*xji*zji
          derv2(ixf,jy) = derv2(ixf,jy) - d2eijdr2*yji*xji
          derv2(iyf,jy) = derv2(iyf,jy) - d2eijdr2*yji*yji
          derv2(izf,jy) = derv2(izf,jy) - d2eijdr2*yji*zji
          derv2(ixf,jz) = derv2(ixf,jz) - d2eijdr2*zji*xji
          derv2(iyf,jz) = derv2(iyf,jz) - d2eijdr2*zji*yji
          derv2(izf,jz) = derv2(izf,jz) - d2eijdr2*zji*zji
...
!
        if (abs(d2eijdcnidrij).gt.gfnff_cnc6tol.or.abs(d2eijdcnjdrij).gt.gfnff_cnc6tol) then
          call gfnff_drv2_dcnpd_x(lilocal,ljlocal,i,ix,iy,iz,ixf,iyf,izf,j,jx,jy,jz,jxf,jyf,jzf,xji,yji,zji, &
...
!
  if (ldohbcn) then
    ...
    if (status/=0) call deallocate_error('gfnff_phond','cn_hb')
  endif
!
  deallocate(d2gwdcn2,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','d2gwdcn2')
  deallocate(dgwdcn,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','dgwdcn')
  deallocate(gw,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','gw')
  deallocate(d2cn,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','d2cn')
  deallocate(dcn,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','dcn')
  deallocate(cn,stat=status)
  if (status/=0) call deallocate_error('gfnff_phond','cn')
!
  time2 = g_cpu_time()
  tgfnff = tgfnff + time2 - time1
#ifdef TRACE
  call trace_out('gfnff_phond')
#endif
!
  return
  end
