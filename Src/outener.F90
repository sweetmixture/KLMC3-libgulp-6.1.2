...
!
  ...
  etot = erecip + eatom + sft + ethb + ereal + efor + epv + ecmm + ec6 + eoop + emany + &
         ...
         eedip + emad + eimp + ekim + erelax + eplumed + eval + egfnff
!
  if (lfree) then
  ...
  if (lgfnff) then
    write(ioout,'(''  GFNFF force field          = '',f20.8,1x,a6)') egfnff*units,unitword
  endif
  if (nvalener.gt.0) then
    write(ioout,'(''  Valence bond force field   = '',f20.8,1x,a6)') eval*units,unitword
  endif
  if (nboQ.gt.0) then
  ...
  if (abs(eqeq).gt.1.0d-8) then
    ...
    elseif (lgfnff) then
      write(ioout,'(''  GFNFF Coulomb correction   = '',f20.8,1x,a6)') eqeq*units,unitword
    else
