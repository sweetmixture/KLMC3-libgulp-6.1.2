...
!  11/21 Keyword to allow centre of mass motion added
!   8/22 Modifications made to handle case where user has specified velocities
!
!  Conditions of use:
!
...
!
!  Set up TI if required
!
  if (ltirun) then
    nlambdanow = 1
    ntistep = nint(tlambda/tstep(ncf))
    ntistep = max(ntistep,1_i4)
    rtistep = 1.0_dp/dble(ntistep)
    lambda = lambda_initial
    dUdlambda(nlambdanow) = 0.0_dp
    lambda_step = (lambda_final - lambda_initial)/dble(nlambda)
  endif
!
  if (lmain) then
!******************
...
!
      ...
      ndof_baro = 0
!
!  For clusters remove rotation
!
      if (ndim.eq.0.and..not.lmd_inc_rot) then
        ndof_atm = ndof_atm - 3
      endif
...
!*************************************************************
    call mdvelcor(ndim.eq.0.and..not.lmd_inc_rot,.false.,.false.)
!
