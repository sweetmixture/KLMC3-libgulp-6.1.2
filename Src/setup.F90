...
!
  ...
  write(*,'(A,I8,I8,F24.12)') "in setup.F90 (0) : ncf length dielectriccfg(ncf) : ", ncf, size(dielectriccfg), dielectriccfg(ncf)
  ! wkjee - in the first  gulpmain call : array(?) dielectriccfg works fine
  ! wkjee - in the second gulpmain call : array(?) dielectriccfg corrupted
  ! wkjee - check if freed?
  if(associated(dielectriccfg)) then
    write(*,'(A,I8,F24.12)') "in setup.F90 (0)-1 : still allocated! : ncf dielectriccfg(ncf) : ", ncf, dielectriccfg(ncf)
    ! wkjee - in the second gulp call, it is still allocated!
  else
    write(*,'(A)') "in setup.F90 (0)-1 : associated .false."
  end if
  ! wkjee - end
#endif
!
