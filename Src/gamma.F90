...
!  Gamma  *
!**********
  subroutine gammas(na,nb,za,zb,r,gam,dgam,dza,dzb,d2zaa,d2zab,d2zbb,d2zra,d2zrb,d2gam)
!
...
!  d2gam    =  second derivative of gam with respect to r
!
!  Julian Gale, CIC, Curtin University, March 2021
!
  use datatypes
  use control,       only : lwolf, lwolffennell
...
!
  ...
  d2gam = d2gam + 0.5_dp*dble(na*(na2-1))*d2rtrm*ss + 2.0_dp*dble(na)*drtrm*deriv + rtrm*deriv2
!
!  Sum over 2*nb
!
  rtrm = drtrm
  ...
  do i = nb2,1,-1
    ...
    d2gam = d2gam - trm2*deriv2 - trm3*deriv - 0.5_dp*trm3*dble(na2+nb2-i-1)*ss/r
    dza = dza - trm2*dzeta1
...
!
  if (r.gt.rqeqtaper) then
    call mdftaper(r,rqeqtaper,rqeq,tpfn,dtpfn,d2tpfn,d3tpfn,.true.,.true.,.false.)
    if (lwolf) then
!
!  Compute Wolf terms if specified
!
      if (r.lt.cutw) then
        t_erfc = g_derfc(etaw*r)
        dt_erfc = - 2.0_dp*etaw*exp(-(etaw*r)**2)/sqrt(pi)
        d2t_erfc = - 2.0_dp*dt_erfc*r*etaw**2
        gamrr = t_erfc/r - selfwolf
        dgamrr = dt_erfc/r - t_erfc/r**2
        d2gamrr = d2t_erfc/r - 2.0_dp*dt_erfc/r**2 + 2.0_dp*t_erfc/r**3
        if (lwolffennell) then
          gamrr = gamrr + selfwolfenergy*(r - cutw)
        endif
        dgamrr = dgamrr + selfwolfforce
      else
        gamrr = 0.0_dp
        dgamrr = 0.0_dp
        d2gamrr = 0.0_dp
...
!
    d2zra = d2zra*tpfn + dza*dtpfn
    d2zrb = d2zrb*tpfn + dzb*dtpfn
    dza = dza*tpfn
    dzb = dzb*tpfn
    d2zaa = d2zaa*tpfn
    d2zab = d2zab*tpfn
