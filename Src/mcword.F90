...
!
  ...
  if (index(word,'mcsp').eq.1) goto 300
  return
!*************************
...
!**************************
300 if (nfloat.eq.0) then
    call outerror('transform probability is missing from input',iline)
    call stopnow('mcword')
  endif
!
!  Increment number of mctrans
!
  nmctrans = nmctrans + 1
!
!  Check dimensions
!
  if (nmctrans.gt.maxmctrans) then
    maxmctrans = nmctrans + 2_i4
    call changemaxmctrans
  endif
!
  ptran(nmctrans) = floats(1)
  ptransum = ptransum + ptran(nmctrans)
!
!  Read species list
!
  n = 0
...
!
      call ltont(words(i),inat,itype)
      nmctrannat(n,nmctrans) = inat
      nmctrantype(n,nmctrans) = itype
    endif
    if (n.eq.2) exit
  enddo
!
...
!
  if (n.eq.0) then
    call outerror('number of species specified for transformation is zero',iline)
    call stopnow('mcword')
  endif
  lwordok = .true.
  return
