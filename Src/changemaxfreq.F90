...
!
  ...
  use frequencies,    only : groupvelocity, grueneisen, fmass, rfmass, aphase, fphase
  use reallocate
  implicit none
!
!  Passed variables
...
!
    ...
    call realloc(aphase,maxfreq,ierror)
    if (ierror.ne.0) call outofmemory('changemaxfreq','aphase')
    call realloc(fphase,3_i4*maxfreq,ierror)
    if (ierror.ne.0) call outofmemory('changemaxfreq','aphase')
  endif
!
  return
