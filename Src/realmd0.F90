...
!
      ...
      ! wkjee factor check
      !write(*,'(A,F24.12,F24.12,F24.12)')        "in realmd0.F90: factor 1 : oci    ocj angstoev ", oci, ocj, angstoev
      !write(*,'(A,F24.12,F24.12,F24.12,F24.12)') "in realmd0.F90: factor 1 : factor qli  qlj  fct", factor, qli, qlj, fct
!
...
!*************************************************
      ...
      ec6_before   = ec6
      ! wkjee
      !write(ioout,'(A)') "wkjee realmd0 flag 2"
      !write(ioout,'(A,F40.12)') "before calling ereal: ", ereal
      !write(ioout,'(A,F40.12)') "before calling twobody1: factor = ", factor
...
!
      ...
      if (lPrintTwo) then
        ! wkjee
        !write(ioout,'(A)') "wkjee realmd0 flag 1"
        !write(ioout,'(A,F40.12)') "eatom          :", eatom
        !write(ioout,'(A,F40.12)') "eatom_before   :", eatom_before
        !write(ioout,'(A,F40.12)') "ec6            :", ec6
        !write(ioout,'(A,F40.12)') "ec6_before     :", ec6_before
        !write(ioout,'(A,F40.12)') "ereal          :", ereal
        !write(ioout,'(A,F40.12)') "ereal_before   :", ereal_before
