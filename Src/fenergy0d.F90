...
#ifdef ELPA
  if (lelpa) then
    call eigensolve_elpa(mcv,3_i4*nblocksize,derv2,maxd2,dervi,maxd2,freq,.true.)
  else
#endif
!
  ...
  if (lnozeropt) then
    if (lequipartition) then
      do iloc = mcvminloc,mcvmaxloc
        i = mlocptr(iloc)
        fetrm = fetrm + log(freq(i,1)*cmfact)
        dfdw2(iloc) = 0.5_dp*rkptfct/freq(i,1)**2
    ...
    if (lequipartition) then
      do iloc = mcvminloc,mcvmaxloc
        ...
        fetrm = fetrm + log(freq(i,1)*cmfact)
        dfdw2(iloc) = 0.25_dp*rkptcmfct/freq(i,1) + 0.5_dp*rkptfct/freq(i,1)**2
        dfdw2(iloc) = sqrt(dfdw2(iloc))*fscale
      enddo
    else
      do iloc = mcvminloc,mcvmaxloc
