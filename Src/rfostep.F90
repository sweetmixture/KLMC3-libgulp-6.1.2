...
#ifdef ELPA
    if (lelpa) then
      call eigensolve_elpa(nvar,nblocksizevar,hessian,maxhess,derv2,maxd2,eig,.true.)
    else
#endif
!
...
!
      ldm = maxd2
      call descinit( idescd, nv, nv, nb, nb, 0, 0, iBlacsContext, ldm, ifails )
!
      if (ifails.ne.0) then
...
!
      lwrk = -1
      call pdsyev('V','U',nv,derv2,1,1,idescd,eig,hessave,1,1,idescd,wrk,lwrk,ierr)
      lwrk = 2*nint(wrk(1))
