...
!        is specified
!   3/21 Wolf sum modification added by adding cutoff to real space
...
!
!  Check Wolf cutoff if appropriate
!
            if (lwolf) then
              if (rr2.gt.cut2w) cycle kkloop
            endif
!
!  Trap self term
!
            if (rr2.lt.cuts2) cycle kkloop
!
            rl = sqrt(rr2)
            if (lqeq) then
!
