...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine gfnff_hbp(nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xkv,ykv,zkv)
!
!  Compute the hydrogen bond phonons without the use of list based methods
!  so that periodic boundary conditions can be handled correctly.
!
!   3/21 Created from gfnff_ehb
!
...
!
            ...
            if (nat(j).eq.8.and.loneneighC) then
              ...
              call gfnff_hb2_eg3p(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij, &
                                  const,radij,xkv,ykv,zkv)
!
...
!
            elseif (nat(j).eq.7.and.nnbr(j).eq.2) then
              ...
              call gfnff_hb2_eg2p_rnr(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij, &
                                      const,radij,xkv,ykv,zkv)
            else
!
!  Default
!
              const = gfnff_hb_acid(i)*qhba*gfnff_hb_base(j)*qhbb*gfnff_hb_scale_gen
              call gfnff_hb2_eg2p(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij, &
                                  const,radij,xkv,ykv,zkv)
            endif
          enddo
...
!
              ...
              if (nat(i).eq.8.and.loneneighC) then
                ...
                call gfnff_hb2_eg3p(j,i,nj,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,-xij,-yij,-zij,rij, &
                                    const,radij,xkv,ykv,zkv)
!
!  Nitro case R-N=O...H_A
!
...
!
              elseif (nat(i).eq.7.and.nnbr(i).eq.2) then
                ...
                call gfnff_hb2_eg2p_rnr(j,i,nj,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,-xij,-yij,-zij,rij, &
                                        const,radij,xkv,ykv,zkv)
              else
!
!  Default
!
                const = gfnff_hb_base(i)*qhba*gfnff_hb_acid(j)*qhbb*gfnff_hb_scale_gen
                call gfnff_hb2_eg2p(j,i,nj,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,-xij,-yij,-zij,rij, &
                                    const,radij,xkv,ykv,zkv)
              endif
            enddo
          endif
...
!
            ...
            call add_drv2_1p(ix,iy,iz,jx,jy,jz,xij,yij,zij,deijkdrij,d2trm,d2r2dx23(1,1))
!
!  i-j / i-k
!
            d2trm = - const*(dabdrik*(drdampdrij*dampo + rdamp*ddampodrij) + &
                             acid*base*(drdampdrij*ddampodrik + rdamp*d2dampo(2)))
            call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,kx,ky,kz,xij,yij,zij,xik,yik,zik,d2trm)
!
!  i-j / j-k
!
            d2trm = - const*(dabdrjk*(drdampdrij*dampo + rdamp*ddampodrij) + &
                             acid*base*(drdampdrij*ddampodrjk + rdamp*d2dampo(3)))
            call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,kx,ky,kz,xij,yij,zij,xjk,yjk,zjk,d2trm)
!
!  i-k / i-k
!
            d2trm = - const*rdamp*(d2abdrik2*dampo + 2.0_dp*dabdrik*ddampodrik + acid*base*d2dampo(4))
            call add_drv2_1p(ix,iy,iz,kx,ky,kz,xik,yik,zik,deijkdrik,d2trm,d2r2dx23(1,2))
!
!  i-k / j-k
!
            d2trm = - const*rdamp*(d2abdrikdrjk*dampo + dabdrik*ddampodrjk + dabdrjk*ddampodrik + acid*base*d2dampo(5))
            call add_drv2_2p(ix,iy,iz,kx,ky,kz,jx,jy,jz,kx,ky,kz,xik,yik,zik,xjk,yjk,zjk,d2trm)
!
!  j-k / j-k
!
            d2trm = - const*rdamp*(d2abdrjk2*dampo + 2.0_dp*dabdrjk*ddampodrjk + acid*base*d2dampo(6))
            call add_drv2_1p(jx,jy,jz,kx,ky,kz,xjk,yjk,zjk,deijkdrjk,d2trm,d2r2dx23(1,3))
...
!
  ...
  enddo
!
  end subroutine gfnff_hbp
!
  subroutine gfnff_hb2_eg2p(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij,const_in,radij,xkv,ykv,zkv)
!
!  Calculates the hydrogen bonding phonons for the GFNFF force fields for nhb2
!
!  On entry : 
!
...
!
!   3/21 Created from gfnff_hb2_eg2
!
!  Conditions of use:
!
...
!
  ...
  real(dp)                                         :: zjm
#ifdef TRACE
  call trace_in('gfnff_hb2_eg2p')
#endif
!
  allocate(dampo_nb(maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p','dampo_nb')
  allocate(ddampo_nbdr(3,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p','ddampo_nbdr')
  allocate(d2dampo_nbdr2(6,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p','d2dampo_nbdr2')
!
  k = nbrno(ni,i)   ! H
!
...
!
    ...
    call add_drv2_1p(ix,iy,iz,jx,jy,jz,xij,yij,zij,deijkdrij,d2trm,d2r2dx23l(1,1))
!
!  i-j / i-l
!
    d2trm = - const*rdamp*dampo*dampo_nb_totm1*d2dampo_nbdr2(2,nj)
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,lx,ly,lz,xij,yij,zij,xil,yil,zil,d2trm)
!
!  i-j / j-l
!
    d2trm = - const*rdamp*dampo*dampo_nb_totm1*d2dampo_nbdr2(3,nj)
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,lx,ly,lz,xij,yij,zij,xjl,yjl,zjl,d2trm)
!
!  i-l / i-l
!
    d2trm = - const*rdamp*dampo*dampo_nb_totm1*d2dampo_nbdr2(4,nj)
    call add_drv2_1p(ix,iy,iz,lx,ly,lz,xil,yil,zil,deijkdril,d2trm,d2r2dx23l(1,2))
!
!  i-l / j-l
!
    d2trm = - const*rdamp*dampo*dampo_nb_totm1*d2dampo_nbdr2(5,nj)
    call add_drv2_2p(ix,iy,iz,lx,ly,lz,jx,jy,jz,lx,ly,lz,xil,yil,zil,xjl,yjl,zjl,d2trm)
!
!  j-l / j-l
!
    d2trm = - const*rdamp*dampo*dampo_nb_totm1*d2dampo_nbdr2(6,nj)
    call add_drv2_1p(jx,jy,jz,lx,ly,lz,xjl,yjl,zjl,deijkdrjl,d2trm,d2r2dx23l(1,3))
!------------------------------------------------
...
!
    ...
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,jx,jy,jz,xij,yij,zij,xij,yij,zij,d2trm)
!
!  i-j / i-l
!
...
!
    ...
    call add_drv2_2p(ix,iy,iz,kx,ky,kz,ix,iy,iz,jx,jy,jz,xik,yik,zik,xij,yij,zij,d2trm)
!
!  i-k / i-l
!
    d2trm = - const*dampo_nb_totm1*ddampo_nbdr(2,nj)*rdamp*ddampodrik
    call add_drv2_2p(ix,iy,iz,kx,ky,kz,ix,iy,iz,lx,ly,lz,xik,yik,zik,xil,yil,zil,d2trm)
!
!  i-k / j-l
!
    d2trm = - const*dampo_nb_totm1*ddampo_nbdr(3,nj)*rdamp*ddampodrik
    call add_drv2_2p(ix,iy,iz,kx,ky,kz,jx,jy,jz,lx,ly,lz,xik,yik,zik,xjl,yjl,zjl,d2trm)
!
!  j-k / i-j
!
    d2trm = - const*dampo_nb_totm1*ddampo_nbdr(1,nj)*(rdamp*ddampodrjk + drdampdrjk*dampo)
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,ix,iy,iz,jx,jy,jz,xjk,yjk,zjk,xij,yij,zij,d2trm)
!
!  j-k / i-l
!
    d2trm = - const*dampo_nb_totm1*ddampo_nbdr(2,nj)*(rdamp*ddampodrjk + drdampdrjk*dampo)
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,ix,iy,iz,lx,ly,lz,xjk,yjk,zjk,xil,yil,zil,d2trm)
!
!  j-k / j-l
!
    d2trm = - const*dampo_nb_totm1*ddampo_nbdr(3,nj)*(rdamp*ddampodrjk + drdampdrjk*dampo)
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,lx,ly,lz,xjk,yjk,zjk,xjl,yjl,zjl,d2trm)
!
...
!
      ...
      call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,mx,my,mz,xij,yij,zij,xim,yim,zim,d2trm)
!
!  i-j / j-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(1,nj)*ddampo_nbdr(3,nk)
      call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,mx,my,mz,xij,yij,zij,xjm,yjm,zjm,d2trm)
!
!  i-l / i-j
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(2,nj)*ddampo_nbdr(1,nk)
      call add_drv2_2p(ix,iy,iz,lx,ly,lz,ix,iy,iz,jx,jy,jz,xil,yil,zil,xij,yij,zij,d2trm)
!
!  i-l / i-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(2,nj)*ddampo_nbdr(2,nk)
      call add_drv2_2p(ix,iy,iz,lx,ly,lz,ix,iy,iz,mx,my,mz,xil,yil,zil,xim,yim,zim,d2trm)
!
!  i-l / j-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(2,nj)*ddampo_nbdr(3,nk)
      call add_drv2_2p(ix,iy,iz,lx,ly,lz,jx,jy,jz,mx,my,mz,xil,yil,zil,xjm,yjm,zjm,d2trm)
!
!  j-l / i-j
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(3,nj)*ddampo_nbdr(1,nk)
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,ix,iy,iz,jx,jy,jz,xjl,yjl,zjl,xij,yij,zij,d2trm)
!
!  j-l / i-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(3,nj)*ddampo_nbdr(2,nk)
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,ix,iy,iz,mx,my,mz,xjl,yjl,zjl,xim,yim,zim,d2trm)
!
!  j-l / j-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm2*ddampo_nbdr(3,nj)*ddampo_nbdr(3,nk)
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,mx,my,mz,xjl,yjl,zjl,xjm,yjm,zjm,d2trm)
    enddo
  enddo
!
...
!
  ...
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p','d2dampo_nbdr2')
  deallocate(ddampo_nbdr,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p','ddampo_nbdr')
  deallocate(dampo_nb,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p','dampo_nb')
!
#ifdef TRACE
  call trace_out('gfnff_hb2_eg2p')
#endif
!
  return
  end
!
  subroutine gfnff_hb2_eg2p_rnr(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij, &
                               const_in,radij,xkv,ykv,zkv)
!
!  Calculates the hydrogen phonons for the GFNFF force fields for nhb2
!  Lone pair version.
!
!  On entry : 
!
...
!  const_in        = energy factor
!
!   3/21 Created from gfnff_hb2_eg2_rnr
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
#ifdef TRACE
  call trace_in('gfnff_hb2_eg2p_rnr')
#endif
!
  allocate(dampo_nb(maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p_rnr','dampo_nb')
  allocate(ddampo_nbdr(3,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p_rnr','ddampo_nbdr')
  allocate(d2dampo_nbdr2(6,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg2p_rnr','d2dampo_nbdr2')
!
  k = nbrno(ni,i)   ! H
!
  p_bh = (1.0_dp + gfnff_hbabmix)*gfnff_autoangs**3
...
!
  call add_drv2_1p(ix,iy,iz,jx,jy,jz,xalp,yalp,zalp,dtrmalp,d2trmalp2,d2r2dx2ra)
!
!  Mixed terms
!
!  i-j / i-j
!
  ...
  call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,jx,jy,jz,xij,yij,zij,xalp,yalp,zalp,d2trm)
!
...
!
  ...
  call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,kx,ky,kz,xalp,yalp,zalp,xik,yik,zik,d2trm)
!
!  i-j / j-k
!
...
!
  ...
  call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,kx,ky,kz,xalp,yalp,zalp,xjk,yjk,zjk,d2trm)
!
!  i-k / i-k
!
...
!
    call add_drv2_1gp(jx,jy,jz,lx,ly,lz,dr2alp(1),dr2alp(2),dr2alp(3),dtrmalp,d2trmalp2,d2r2blpdx2)
!
...
!
    call add_drv2_2sgp(ix,iy,iz,jx,jy,jz,jx,jy,jz,lx,ly,lz,dtrmalp,dr2alpij,dr2alp,d2trmalp2,d2r2dx2mix)
!
!  Mixed terms
!
!  i-j / j-l (ralp)
!
    d2trm = - const*dampo_nb_tot*(dampo*rdamp*d2dampo_lp(2) + dampo*drdampdrij*ddampo_lpdralp + &
              rdamp*ddampodrij*ddampo_lpdralp)
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,lx,ly,lz,xij,yij,zij,dr2alp(1),dr2alp(2),dr2alp(3),d2trm)
!
!  j-l (ralp)/ i-k
!
    d2trm = - const*dampo_nb_tot*rdamp*ddampodrik*ddampo_lpdralp
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,ix,iy,iz,kx,ky,kz,dr2alp(1),dr2alp(2),dr2alp(3),xik,yik,zik,d2trm)
!
!  j-l (ralp)/ j-k
!
    d2trm = - const*dampo_nb_tot*(rdamp*ddampodrjk + dampo*drdampdrjk)*ddampo_lpdralp
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,kx,ky,kz,dr2alp(1),dr2alp(2),dr2alp(3),xjk,yjk,zjk,d2trm)
!
    do nk = 1,nj-1
      m = nbrno(nk,j)
      indm = 3*(m-1)
...
!
      call add_drv2_2sgnosp(jx,jy,jz,lx,ly,lz,jx,jy,jz,mx,my,mz,dtrmalp,dr2alp,dr2alp, &
...
!
      ...
      call add_drv2_1p(ix,iy,iz,jx,jy,jz,xij,yij,zij,deijkdrij,d2trm,d2r2dx23m(1,1))
!
!  i-j / i-m
!
...
!
      ...
      call add_drv2_1p(ix,iy,iz,mx,my,mz,xim,yim,zim,deijkdrim,d2trm,d2r2dx23m(1,2))
!
!  i-m / j-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm1*dampo_lp*d2dampo_nbdr2(5,nj)
      call add_drv2_2p(ix,iy,iz,mx,my,mz,jx,jy,jz,mx,my,mz,xim,yim,zim,xjm,yjm,zjm,d2trm)
!
!  j-m / j-m
!
      d2trm = - const*rdamp*dampo*dampo_nb_totm1*dampo_lp*d2dampo_nbdr2(6,nj)
      call add_drv2_1p(jx,jy,jz,mx,my,mz,xjm,yjm,zjm,deijkdrjm,d2trm,d2r2dx23m(1,3))
!------------------------------------------------
!  Contribution from dampo_nb and 1 other term  |
!------------------------------------------------
!
...
!
      ...
      call add_drv2_2p(ix,iy,iz,kx,ky,kz,ix,iy,iz,mx,my,mz,xik,yik,zik,xim,yim,zim,d2trm)
!
!  i-k / j-m
!
      d2trm = - const*dampo_nb_totm1*dampo_lp*ddampo_nbdr(3,nj)*rdamp*ddampodrik
      call add_drv2_2p(ix,iy,iz,kx,ky,kz,jx,jy,jz,mx,my,mz,xik,yik,zik,xjm,yjm,zjm,d2trm)
!
!  j-k / i-j
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,kx,ky,kz,ix,iy,iz,mx,my,mz,xjk,yjk,zjk,xim,yim,zim,d2trm)
!
!  j-k / j-m
!
      d2trm = - const*dampo_nb_totm1*dampo_lp*ddampo_nbdr(3,nj)*(rdamp*ddampodrjk + drdampdrjk*dampo)
      call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,mx,my,mz,xjk,yjk,zjk,xjm,yjm,zjm,d2trm)
!--------------------------------------------------------
...
!
      ...
      call add_drv2_2p(ix,iy,iz,mx,my,mz,ix,iy,iz,jx,jy,jz,xim,yim,zim,xij,yij,zij,d2trm)
!
!  j-m / i-j
!
      d2trm = - const*dampo_nb_totm1*dampo*rdamp*ddampo_nbdr(3,nj)*ddampo_lpdrij
      call add_drv2_2p(jx,jy,jz,mx,my,mz,ix,iy,iz,jx,jy,jz,xjm,yjm,zjm,xij,yij,zij,d2trm)
!
!  i-j / i-j (ralp)
!
...
!
      ...
      call add_drv2_2p(ix,iy,iz,mx,my,mz,ix,iy,iz,jx,jy,jz,xim,yim,zim,xalp,yalp,zalp,d2trm)
!
!  j-m / i-j (ralp)
!
      d2trm = - const*dampo_nb_totm1*dampo*rdamp*ddampo_nbdr(3,nj)*ddampo_lpdralp
      call add_drv2_2p(jx,jy,jz,mx,my,mz,ix,iy,iz,jx,jy,jz,xjm,yjm,zjm,xalp,yalp,zalp,d2trm)
!
...
!
        ...
        call add_drv2_2p(jx,jy,jz,lx,ly,lz,ix,iy,iz,jx,jy,jz,dr2alp(1),dr2alp(2),dr2alp(3),xij,yij,zij,d2trm)
!
!  j-l (ralp)/ i-m
!
        d2trm = - const*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(2,nj)*ddampo_lpdralp
        call add_drv2_2p(jx,jy,jz,lx,ly,lz,ix,iy,iz,mx,my,mz,dr2alp(1),dr2alp(2),dr2alp(3),xim,yim,zim,d2trm)
!
!  j-l (ralp)/ j-m
!
        d2trm = - const*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(3,nj)*ddampo_lpdralp
        call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,mx,my,mz,dr2alp(1),dr2alp(2),dr2alp(3),xjm,yjm,zjm,d2trm)
      enddo
!----------------------------------------------------------------------
!  Loop over second neighbour of B for products of first derivatives  |
!----------------------------------------------------------------------
...
!
        ...
        call add_drv2_2p(ix,iy,iz,jx,jy,jz,ix,iy,iz,nx,ny,nz,xij,yij,zij,xin,yin,zin,d2trm)
!
!  i-j / j-n
!
        d2trm = - const*rdamp*dampo*dampo_nb_totm2*dampo_lp*ddampo_nbdr(1,nj)*ddampo_nbdr(3,nk)
        call add_drv2_2p(ix,iy,iz,jx,jy,jz,jx,jy,jz,nx,ny,nz,xij,yij,zij,xjn,yjn,zjn,d2trm)
!
!  i-m / i-j
!
...
!
        ...
        call add_drv2_2p(ix,iy,iz,mx,my,mz,ix,iy,iz,nx,ny,nz,xim,yim,zim,xin,yin,zin,d2trm)
!
!  i-m / j-n
!
        d2trm = - const*rdamp*dampo*dampo_nb_totm2*dampo_lp*ddampo_nbdr(2,nj)*ddampo_nbdr(3,nk)
        call add_drv2_2p(ix,iy,iz,mx,my,mz,jx,jy,jz,nx,ny,nz,xim,yim,zim,xjn,yjn,zjn,d2trm)
!
!  j-m / i-j
!
...
!
        ...
        call add_drv2_2p(jx,jy,jz,mx,my,mz,ix,iy,iz,nx,ny,nz,xjm,yjm,zjm,xin,yin,zin,d2trm)
!
!  j-m / j-n
!
        d2trm = - const*rdamp*dampo*dampo_nb_totm2*dampo_lp*ddampo_nbdr(3,nj)*ddampo_nbdr(3,nk)
        call add_drv2_2p(jx,jy,jz,mx,my,mz,jx,jy,jz,nx,ny,nz,xjm,yjm,zjm,xjn,yjn,zjn,d2trm)
      enddo
    enddo
  endif
!
...
!
  deallocate(d2dampo_nbdr2,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p_rnr','d2dampo_nbdr2')
  deallocate(ddampo_nbdr,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p_rnr','ddampo_nbdr')
  deallocate(dampo_nb,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg2p_rnr','dampo_nb')
!
#ifdef TRACE
  call trace_out('gfnff_hb2_eg2p_rnr')
#endif
!
  return
  end
!
  subroutine gfnff_hb2_eg3p(i,j,ni,nnbr,maxnbr,nbrno,rbnbr,xbnbr,ybnbr,zbnbr,xij,yij,zij,rij, &
                            const_in,radij,xkv,ykv,zkv)
!
!  Calculates the hydrogen bonding phonons for the GFNFF force fields for nhb2
...
!  const_in        = energy factor
!
!   3/21 Created from gfnff_hb2_eg3
!
!  Conditions of use:
!
!  GULP is available free of charge to academic institutions
...
!
  ...
  real(dp)                                         :: zln
#ifdef TRACE
  call trace_in('gfnff_hb2_eg3p')
#endif
!
  allocate(dampo_nb(maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','dampo_nb')
  allocate(ddampo_nbdr(3,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','ddampo_nbdr')
  allocate(d2dampo_nbdr2(6,maxnbr),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','d2dampo_nbdr2')
!
  k = nbrno(ni,i)   ! H
!
  p_bh = (1.0_dp + gfnff_hbabmix)*gfnff_autoangs**3
...
!
  ...
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','ft')
  allocate(dft(6,ntorsion),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','dft')
  allocate(d2ft(21,ntorsion),stat=status)
  if (status/=0) call outofmemory('gfnff_hb2_eg3p','d2ft')
!
...
!
  ...
  call add_drv2_1p(jx,jy,jz,kx,ky,kz,xjk,yjk,zjk,dfangledrjk,d2trm,d2r2dx23a(1,1))
!
!  j-k / j-l
!
...
!
  ...
  call add_drv2_2p(jx,jy,jz,kx,ky,kz,kx,ky,kz,lx,ly,lz,xjk,yjk,zjk,xkl,ykl,zkl,d2trm)
!
!  j-l / j-l
!
  d2trm = - const*ftors*rdamp*qhdampo*(dfangledcosa*d2cos(4) + d2fangledcosa2*dcosdrjl*dcosdrjl)
  call add_drv2_1p(jx,jy,jz,lx,ly,lz,xjl,yjl,zjl,dfangledrjl,d2trm,d2r2dx23a(1,2))
!
!  j-l / k-l
!
  d2trm = - const*ftors*rdamp*qhdampo*(dfangledcosa*d2cos(5) + d2fangledcosa2*dcosdrjl*dcosdrkl)
  call add_drv2_2p(jx,jy,jz,lx,ly,lz,kx,ky,kz,lx,ly,lz,xjl,yjl,zjl,xkl,ykl,zkl,d2trm)
!
!  k-l / k-l
!
  d2trm = - const*ftors*rdamp*qhdampo*(dfangledcosa*d2cos(6) + d2fangledcosa2*dcosdrkl*dcosdrkl)
  call add_drv2_1p(kx,ky,kz,lx,ly,lz,xkl,ykl,zkl,dfangledrkl,d2trm,d2r2dx23a(1,3))
!--------------------------------------------------------
...
!
  ...
  call add_drv2_2p(ix,iy,iz,jx,jy,jz,kx,ky,kz,lx,ly,lz,xij,yij,zij,xkl,ykl,zkl,d2trm)
!
!  i-k / j-k
!
...
!
  ...
  call add_drv2_2p(ix,iy,iz,kx,ky,kz,kx,ky,kz,lx,ly,lz,xik,yik,zik,xkl,ykl,zkl,d2trm)
!
!  j-k / j-k
!
  d2trm = - const*ftors*dampo_nb_tot*(drdampdrjk*dampo + rdamp*ddampodrjk)*dfangledcosa*dcosdrjk
  call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,kx,ky,kz,xjk,yjk,zjk,xjk,yjk,zjk,d2trm)
!
!  j-k / j-l
!
...
!
    ...
    call add_drv2_2p(ix,iy,iz,mx,my,mz,jx,jy,jz,kx,ky,kz,xim,yim,zim,xjk,yjk,zjk,d2trm)
!
!  i-m / j-l
!
    d2trm = - const*ftors*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(2,nj)*dfangledcosa*dcosdrjl
    call add_drv2_2p(ix,iy,iz,mx,my,mz,jx,jy,jz,lx,ly,lz,xim,yim,zim,xjl,yjl,zjl,d2trm)
!
!  i-m / k-l
!
    d2trm = - const*ftors*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(2,nj)*dfangledcosa*dcosdrkl
    call add_drv2_2p(ix,iy,iz,mx,my,mz,kx,ky,kz,lx,ly,lz,xim,yim,zim,xkl,ykl,zkl,d2trm)
!
!  j-m / j-k
!
    d2trm = - const*ftors*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(3,nj)*dfangledcosa*dcosdrjk
    call add_drv2_2p(jx,jy,jz,mx,my,mz,jx,jy,jz,kx,ky,kz,xjm,yjm,zjm,xjk,yjk,zjk,d2trm)
!
!  j-m / j-l
!
    d2trm = - const*ftors*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(3,nj)*dfangledcosa*dcosdrjl
    call add_drv2_2p(jx,jy,jz,mx,my,mz,jx,jy,jz,lx,ly,lz,xjm,yjm,zjm,xjl,yjl,zjl,d2trm)
!
!  j-m / k-l
!
    d2trm = - const*ftors*dampo_nb_totm1*rdamp*dampo*ddampo_nbdr(3,nj)*dfangledcosa*dcosdrkl
    call add_drv2_2p(jx,jy,jz,mx,my,mz,kx,ky,kz,lx,ly,lz,xjm,yjm,zjm,xkl,ykl,zkl,d2trm)
!
!  Loop over second neighbour of B for products of first derivatives
!
    do nk = 1,nj-1
...
!  j-k / j-k
!
    call add_drv2_1p(jx,jy,jz,kx,ky,kz,xjk,yjk,zjk,e1d(1),e2d(1),d2r2dx26t(1,1))
!
!  j-k / k-l
!
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,kx,ky,kz,lx,ly,lz,xjk,yjk,zjk,xkl,ykl,zkl,e2d(2))
!
!  j-k / k-m
!
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,kx,ky,kz,mx,my,mz,xjk,yjk,zjk,xkm,ykm,zkm,e2d(3))
!
!  j-k / j-l
!
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,lx,ly,lz,xjk,yjk,zjk,xjl,yjl,zjl,e2d(4))
!
!  j-k / j-m
!
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,mx,my,mz,xjk,yjk,zjk,xjm,yjm,zjm,e2d(5))
!
!  j-k / l-m
!
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,lx,ly,lz,mx,my,mz,xjk,yjk,zjk,xlm,ylm,zlm,e2d(6))
!
!  k-l / k-l
!
    call add_drv2_1p(kx,ky,kz,lx,ly,lz,xkl,ykl,zkl,e1d(2),e2d(7),d2r2dx26t(1,2))
!
!  k-l / k-m
!
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,kx,ky,kz,mx,my,mz,xkl,ykl,zkl,xkm,ykm,zkm,e2d(8))
!
!  k-l / j-l
!
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,lx,ly,lz,xkl,ykl,zkl,xjl,yjl,zjl,e2d(9))
!
!  k-l / j-m
!
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,mx,my,mz,xkl,ykl,zkl,xjm,yjm,zjm,e2d(10))
!
!  k-l / l-m
!
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,lx,ly,lz,mx,my,mz,xkl,ykl,zkl,xlm,ylm,zlm,e2d(11))
!
!  k-m / k-m
!
    call add_drv2_1p(kx,ky,kz,mx,my,mz,xkm,ykm,zkm,e1d(3),e2d(12),d2r2dx26t(1,3))
!
!  k-m / j-l
!
    call add_drv2_2p(kx,ky,kz,mx,my,mz,jx,jy,jz,lx,ly,lz,xkm,ykm,zkm,xjl,yjl,zjl,e2d(13))
!
!  k-m / j-m
!
    call add_drv2_2p(kx,ky,kz,mx,my,mz,jx,jy,jz,mx,my,mz,xkm,ykm,zkm,xjm,yjm,zjm,e2d(14))
!     
!  k-m / l-m
! 
    call add_drv2_2p(kx,ky,kz,mx,my,mz,lx,ly,lz,mx,my,mz,xkm,ykm,zkm,xlm,ylm,zlm,e2d(15))
!     
!  j-l / j-l
! 
    call add_drv2_1p(jx,jy,jz,lx,ly,lz,xjl,yjl,zjl,e1d(4),e2d(16),d2r2dx26t(1,4))
!
!  j-l / j-m
!
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,mx,my,mz,xjl,yjl,zjl,xjm,yjm,zjm,e2d(17))
!
!  j-l / l-m
!
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,lx,ly,lz,mx,my,mz,xjl,yjl,zjl,xlm,ylm,zlm,e2d(18))
!
!  j-m / j-m
!
    call add_drv2_1p(jx,jy,jz,mx,my,mz,xjm,yjm,zjm,e1d(5),e2d(19),d2r2dx26t(1,5))
!
!  j-m / l-m
!
    call add_drv2_2p(jx,jy,jz,mx,my,mz,lx,ly,lz,mx,my,mz,xjm,yjm,zjm,xlm,ylm,zlm,e2d(20))
!
!  l-m / l-m
!
    call add_drv2_1p(lx,ly,lz,mx,my,mz,xlm,ylm,zlm,e1d(6),e2d(21),d2r2dx26t(1,6))
!-------------------------------------------------------
...
!
    ...
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,kx,ky,kz,mx,my,mz,xij,yij,zij,xkm,ykm,zkm,d2trm)
!
!  i-j / j-l
!
...
!
    ...
    call add_drv2_2p(ix,iy,iz,jx,jy,jz,lx,ly,lz,mx,my,mz,xij,yij,zij,xlm,ylm,zlm,d2trm)
!
!  i-k / j-k
!
...
!
    ...
    call add_drv2_2p(ix,iy,iz,kx,ky,kz,kx,ky,kz,mx,my,mz,xik,yik,zik,xkm,ykm,zkm,d2trm)
!
!  i-k / j-l
!
...
!
    ...
    call add_drv2_2p(ix,iy,iz,kx,ky,kz,lx,ly,lz,mx,my,mz,xik,yik,zik,xlm,ylm,zlm,d2trm)
!
!  j-k / j-k
!
...
!
    ...
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,kx,ky,kz,mx,my,mz,xjk,yjk,zjk,xkm,ykm,zkm,d2trm)
!
!  j-k / j-l
!
...
!
    ...
    call add_drv2_2p(jx,jy,jz,kx,ky,kz,lx,ly,lz,mx,my,mz,xjk,yjk,zjk,xlm,ylm,zlm,d2trm)
!--------------------------------------------------
...
!
    ...
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,kx,ky,kz,xjl,yjl,zjl,xjk,yjk,zjk,d2trm)
!
!  j-l / k-l
!
...
!
    ...
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,kx,ky,kz,mx,my,mz,xjl,yjl,zjl,xkm,ykm,zkm,d2trm)
!
!  j-l / j-l
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrjl*ftorsm1*dft(4,nt)
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,lx,ly,lz,xjl,yjl,zjl,xjl,yjl,zjl,d2trm)
!
!  j-l / j-m
!
...
!
    ...
    call add_drv2_2p(jx,jy,jz,lx,ly,lz,lx,ly,lz,mx,my,mz,xjl,yjl,zjl,xlm,ylm,zlm,d2trm)
!
!  k-l / j-k
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(1,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,kx,ky,kz,xkl,ykl,zkl,xjk,yjk,zjk,d2trm)
!
!  k-l / k-l
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(2,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,kx,ky,kz,lx,ly,lz,xkl,ykl,zkl,xkl,ykl,zkl,d2trm)
!
!  k-l / k-m
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(3,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,kx,ky,kz,mx,my,mz,xkl,ykl,zkl,xkm,ykm,zkm,d2trm)
!
!  k-l / j-l
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(4,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,lx,ly,lz,xkl,ykl,zkl,xjl,yjl,zjl,d2trm)
!
!  k-l / j-m
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(5,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,mx,my,mz,xkl,ykl,zkl,xjm,yjm,zjm,d2trm)
!
!  k-l / l-m
!
    d2trm = - const*rdamp*qhdampo*dfangledcosa*dcosdrkl*ftorsm1*dft(6,nt)
    call add_drv2_2p(kx,ky,kz,lx,ly,lz,lx,ly,lz,mx,my,mz,xkl,ykl,zkl,xlm,ylm,zlm,d2trm)
!
!  Loop over second neighbour of B for products of first derivatives
!
    nt2 = 0
...
!
      ...
      call add_drv2_2p(jx,jy,jz,kx,ky,kz,kx,ky,kz,nx,ny,nz,xjk,yjk,zjk,xkn,ykn,zkn,d2trm)
!
!  j-k / j-l
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,kx,ky,kz,jx,jy,jz,nx,ny,nz,xjk,yjk,zjk,xjn,yjn,zjn,d2trm)
!
!  j-k / l-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(1,nt)*dft(6,nt2)
      call add_drv2_2p(jx,jy,jz,kx,ky,kz,lx,ly,lz,nx,ny,nz,xjk,yjk,zjk,xln,yln,zln,d2trm)
!
!  k-l / j-k
!
...
!
      ...
      call add_drv2_2p(kx,ky,kz,lx,ly,lz,kx,ky,kz,nx,ny,nz,xkl,ykl,zkl,xkn,ykn,zkn,d2trm)
!
!  k-l / j-l
!
...
!
      ...
      call add_drv2_2p(kx,ky,kz,lx,ly,lz,jx,jy,jz,nx,ny,nz,xkl,ykl,zkl,xjn,yjn,zjn,d2trm)
!
!  k-l / l-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(2,nt)*dft(6,nt2)
      call add_drv2_2p(kx,ky,kz,lx,ly,lz,lx,ly,lz,nx,ny,nz,xkl,ykl,zkl,xln,yln,zln,d2trm)
!
!  k-m / j-k
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(1,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,jx,jy,jz,kx,ky,kz,xkm,ykm,zkm,xjk,yjk,zjk,d2trm)
!
!  k-m / k-l
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(2,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,kx,ky,kz,lx,ly,lz,xkm,ykm,zkm,xkl,ykl,zkl,d2trm)
!
!  k-m / k-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(3,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,kx,ky,kz,nx,ny,nz,xkm,ykm,zkm,xkn,ykn,zkn,d2trm)
!
!  k-m / j-l
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(4,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,jx,jy,jz,lx,ly,lz,xkm,ykm,zkm,xjl,yjl,zjl,d2trm)
!
!  k-m / j-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(5,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,jx,jy,jz,nx,ny,nz,xkm,ykm,zkm,xjn,yjn,zjn,d2trm)
!
!  k-m / l-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(3,nt)*dft(6,nt2)
      call add_drv2_2p(kx,ky,kz,mx,my,mz,lx,ly,lz,nx,ny,nz,xkm,ykm,zkm,xln,yln,zln,d2trm)
!
!  j-l / j-k
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,kx,ky,kz,nx,ny,nz,xjl,yjl,zjl,xkn,ykn,zkn,d2trm)
!
!  j-l / j-l
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,jx,jy,jz,nx,ny,nz,xjl,yjl,zjl,xjn,yjn,zjn,d2trm)
!
!  j-l / l-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(4,nt)*dft(6,nt2)
      call add_drv2_2p(jx,jy,jz,lx,ly,lz,lx,ly,lz,nx,ny,nz,xjl,yjl,zjl,xln,yln,zln,d2trm)
!
!  j-m / j-k
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,mx,my,mz,kx,ky,kz,nx,ny,nz,xjm,yjm,zjm,xkn,ykn,zkn,d2trm)
!
!  j-m / j-l
!
...
!
      ...
      call add_drv2_2p(jx,jy,jz,mx,my,mz,lx,ly,lz,nx,ny,nz,xjm,yjm,zjm,xln,yln,zln,d2trm)
!
!  l-m / j-k
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(1,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,jx,jy,jz,kx,ky,kz,xlm,ylm,zlm,xjk,yjk,zjk,d2trm)
!
!  l-m / k-l
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(2,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,kx,ky,kz,lx,ly,lz,xlm,ylm,zlm,xkl,ykl,zkl,d2trm)
!
!  l-m / k-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(3,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,kx,ky,kz,nx,ny,nz,xlm,ylm,zlm,xkn,ykn,zkn,d2trm)
!
!  l-m / j-l
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(4,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,jx,jy,jz,lx,ly,lz,xlm,ylm,zlm,xjl,yjl,zjl,d2trm)
!
!  l-m / j-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(5,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,jx,jy,jz,nx,ny,nz,xlm,ylm,zlm,xjn,yjn,zjn,d2trm)
!
!  l-m / l-n
!
      d2trm = - const*fangle*rdamp*qhdampo*ftorsm2*dft(6,nt)*dft(6,nt2)
      call add_drv2_2p(lx,ly,lz,mx,my,mz,lx,ly,lz,nx,ny,nz,xlm,ylm,zlm,xln,yln,zln,d2trm)
    enddo
!-----------------------------------------------------------
...
!
      ...
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,jx,jy,jz,kx,ky,kz,xin,yin,zin,xjk,yjk,zjk,d2trm)
!
!  i-n / k-l
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(2,nj)*dft(2,nt)
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,kx,ky,kz,lx,ly,lz,xin,yin,zin,xkl,ykl,zkl,d2trm)
!
!  i-n / k-m
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(2,nj)*dft(3,nt)
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,kx,ky,kz,mx,my,mz,xin,yin,zin,xkm,ykm,zkm,d2trm)
!
!  i-n / j-l
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(2,nj)*dft(4,nt)
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,jx,jy,jz,lx,ly,lz,xin,yin,zin,xjl,yjl,zjl,d2trm)
!
!  i-n / j-m
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(2,nj)*dft(5,nt)
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,jx,jy,jz,mx,my,mz,xin,yin,zin,xjm,yjm,zjm,d2trm)
!
!  i-n / l-m
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(2,nj)*dft(6,nt)
      call add_drv2_2p(ix,iy,iz,nx,ny,nz,lx,ly,lz,mx,my,mz,xin,yin,zin,xlm,ylm,zlm,d2trm)
!
!  j-n / j-k
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(1,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,jx,jy,jz,kx,ky,kz,xjn,yjn,zjn,xjk,yjk,zjk,d2trm)
!
!  j-n / k-l
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(2,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,kx,ky,kz,lx,ly,lz,xjn,yjn,zjn,xkl,ykl,zkl,d2trm)
!       
!  j-n / k-m
!   
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(3,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,kx,ky,kz,mx,my,mz,xjn,yjn,zjn,xkm,ykm,zkm,d2trm)
!
!  j-n / j-l
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(4,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,jx,jy,jz,lx,ly,lz,xjn,yjn,zjn,xjl,yjl,zjl,d2trm)
!
!  j-n / j-m
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(5,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,jx,jy,jz,mx,my,mz,xjn,yjn,zjn,xjm,yjm,zjm,d2trm)
!
!  j-n / l-m
!
      d2trm = - const*fangle*rdamp*dampo*dampo_nb_totm1*ftorsm1*ddampo_nbdr(3,nj)*dft(6,nt)
      call add_drv2_2p(jx,jy,jz,nx,ny,nz,lx,ly,lz,mx,my,mz,xjn,yjn,zjn,xlm,ylm,zlm,d2trm)
    enddo
  enddo
!
  deallocate(d2ft,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','d2ft')
  deallocate(dft,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','dft')
  deallocate(ft,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','ft')
!
...
!
  deallocate(d2dampo_nbdr2,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','d2dampo_nbdr2')
  deallocate(ddampo_nbdr,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','ddampo_nbdr')
  deallocate(dampo_nb,stat=status)
  if (status/=0) call deallocate_error('gfnff_hb2_eg3p','dampo_nb')
!
#ifdef TRACE
  call trace_out('gfnff_hb2_eg3p')
#endif
!
  return
  end
