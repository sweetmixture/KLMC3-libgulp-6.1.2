...
!   4/21 Format statements changed to allow for more observables
!
!  Conditions of use:
!
...
!
  if (nfit-nfcon.gt.nobs) then
    if (ioproc) then
      ...
      write(ioout,'(''  **** Number of variables   = '',i8,16x,''****'')') nfit-nfcon
      write(ioout,'(''  **** Number of observables = '',i8,16x,''****'')') nobs
      write(ioout,'(/)')
    endif
    call stopnow('fit')
  endif
...
!
  if (ioproc) then
    write(ioout,'(/)')
    write(ioout,'(''  Number of variables   = '',i8)') nfit
    write(ioout,'(''  Number of observables = '',i8,/)') nobs
  endif
!***********************************
