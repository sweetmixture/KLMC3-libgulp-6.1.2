...
!   7/21 Trap for atomic numbers not being properly defined added
!
!  Conditions of use:
!
...
!
  inat = inatin
!
!  Check that atomic number is sensible
!
  if (inat.eq.0) then
    call outerror('atomic number of zero is not allowed',0_i4)
    call stopnow('label')
  elseif (inat.lt.0) then
    call outerror('negative atomic number is not allowed',0_i4)
    call stopnow('label')
  endif
!
