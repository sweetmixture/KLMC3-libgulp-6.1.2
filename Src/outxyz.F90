...
!   8/21 Extended XYZ file format added
!
!  Conditions of use:
!
...
!
  ...
  character(len=24)            :: energy
  character(len=90)            :: vectors
  character(len=180)           :: xyzline
  integer(i4)                  :: i
  integer(i4)                  :: ii
  integer(i4)                  :: incf
  integer(i4)                  :: ind
...
!
  if (lxyz_extended) then
    write(energy,'(f24.8)') fcsave
    if (ndim.eq.3) then
      write(vectors,'(9f10.5)') rv(1,1),rv(2,1),rv(3,1),rv(1,2),rv(2,2),rv(3,2),rv(1,3),rv(2,3),rv(3,3)
      xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T T T" Energy='&
        //trim(adjustl(energy))
    elseif (ndim.eq.2) then
      write(vectors,'(6f10.5)') rv(1,1),rv(2,1),rv(3,1),rv(1,2),rv(2,2),rv(3,2)
      xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T T F" Energy='&
        //trim(adjustl(energy))
    elseif (ndim.eq.1) then
      write(vectors,'(3f10.5)') rv(1,1),rv(2,1),rv(3,1)
      xyzline = 'Lattice="'//trim(adjustl(vectors))//'" Properties=species:S:1:pos:R:3:charges:R:1 pbc="T F F" Energy='&
        //trim(adjustl(energy))
    else
      xyzline = 'Properties=species:S:1:pos:R:3:charges:R:1 pbc="F F F" Energy='//trim(adjustl(energy))
    endif
    write(iout,'(a)') trim(adjustl(xyzline))
  else
    ...
    if (lxyz_extended) then
      do i = 1,numat
        if (nat(i).le.maxele) then
          ...
          call label(ni,0_i4,lab)
          lab2 = lab(1:4)
          write(iout,'(a4,1x,4f20.9)') lab2,xclat(i),yclat(i),zclat(i),qf(i)
        endif
      enddo
    else
      do i = 1,numat
