...
!   7/21 Correction to self term in parallel
!   7/21 Maxextra added
...
!***********************************
    if (ndim.eq.3.and..not.lwolf) then
      if (.not.lnorecip) then
        ...
        if (status/=0) call outofmemory('genpotd','sinat')
        allocate(cosat(numat),stat=status)
        if (status/=0) call outofmemory('genpotd','cosat')
!
        do iv = 1,nkvec
!
!  Set cos and sin
...
!
        ...
        if (status/=0) call deallocate_error('genpotd','cosat')
        deallocate(sinat,stat=status)
        if (status/=0) call deallocate_error('genpotd','sinat')
      endif
!
...
!
    ...
    if (.not.lqeq.and..not.lSandM.and..not.lgfnff) goto 135
!**************************************************
